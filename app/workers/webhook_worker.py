...
class WebhookWorker:
    ...
    async def start(self):
        ...
        if self._tasks:
            logger.info(
                "webhook_worker_waiting_for_tasks",
                worker_id=self.worker_id,
    ...
    async def _recovery_loop(self):
        ...
        while self._running:
            try:
                ...
                if recovered > 0:
                    logger.info(
                        "webhook_worker_recovered_items",
                        worker_id=self.worker_id,
                        count=recovered
                    )
        ...
        try:
            ...
            if event_type == ChatwootEventType.MESSAGE_CREATED:
                ...
                if webhook.message_type == "incoming":
                    ...
                    logger.debug(
                        "webhook_worker_skipped_outgoing",
                        worker_id=self.worker_id,
                ...
                logger.debug(
                    "webhook_worker_unhandled_event",
                    worker_id=self.worker_id,
                    event_type=event_type
                )
                processing_status = "skipped"
            ...
            chatwoot_message_processing_duration_seconds.labels(
    ...
    async def _process_message(self, webhook: ChatwootMessageWebhook):
        ...
        session_id = MessageMapping.conversation_to_session_id(webhook)
        ...
        for agent_msg in response.get("new_messages", []):
            if agent_msg.role == "assistant" and agent_msg.content:
                ...
                send_tasks.append(
                    self._send_message_safe(conversation.id, chatwoot_message)
                )
        
        # Mark conversation as read (fire-and-forget)
        asyncio.create_task(
            self._mark_read_safe(conversation.id)
        )
    ...
    async def _send_message_safe(self, conversation_id: int, message):
        ...
        """
        try:
            response = await chatwoot_service.send_message(
